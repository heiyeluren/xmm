...
func BenchmarkGcLargeMemory_Go(b *testing.B) {
	...
	fmt.Printf("BenchmarkGcLargeMemory_Go    HeapAlloc:%d GCCPUFraction:%f PauseTotalNs:%d NumGC:%d \n", m.HeapAlloc, m.GCCPUFraction, m.PauseTotalNs, m.NumGC)
}
...
func BenchmarkGcLargeMemory_Xmm(b *testing.B) {
	...
	var m runtime.MemStats
	runtime.ReadMemStats(&m)
	fmt.Printf("BenchmarkGcLargeMemory_Xmm    HeapAlloc:%d GCCPUFraction:%f PauseTotalNs:%d NumGC:%d \n", m.HeapAlloc, m.GCCPUFraction, m.PauseTotalNs, m.NumGC)
}

func TestGcPauseTotal_Xmm(b *testing.T) {
	f := &xmm.Factory{}
	...
	runtime.ReadMemStats(&m)
	fmt.Sprintf("%d", len(dat))
	fmt.Printf("TestGcPauseTotal_Xmm    HeapAlloc:%d GCCPUFraction:%f PauseTotalNs:%d NumGC:%d \n", m.HeapAlloc, m.GCCPUFraction, m.PauseTotalNs, m.NumGC)
}

func TestGcPauseTotal_Go(b *testing.T) {
	var data []byte
	//初始化分配800m
	data2 := make([]*User, 10000)
	for i := 0; i < 10000; i++ {
	...
	runtime.ReadMemStats(&m)
	fmt.Sprintf("%d", len(dat))
	fmt.Printf("TestGcPauseTotal_Go    HeapAlloc:%d GCCPUFraction:%f PauseTotalNs:%d NumGC:%d \n", m.HeapAlloc, m.GCCPUFraction, m.PauseTotalNs, m.NumGC)
}
